function RainyDay(a) {
    ...
    if ("img" === b.tagName.toLowerCase()) this.imgSource = null, this.img = b, this.initialize(a);
    else {
        var c = this,
        ...
        }, f.src = e, c.bckStyle = d
    }
}

function Drop(a, b, c, d, e) {
    this.x = Math.floor(b), this.y = Math.floor(c), this.r = Math.ceil(Math.random() * e + d), this.rainyday = a, this.context = a.context, this.reflection = a.reflected
}
...
function downscaleImage(a, b) {
    var c = document.createElement("canvas"),
        d = c.getContext("2d");
    return c.width = b || 50, c.height = c.width * a.height / a.width, d.drawImage(a, 0, 0, a.width, a.height, 0, 0, c.width, c.height), c
}
...
RainyDay.prototype.destroy = function() {
    this.pause(), this.canvas.parentNode.removeChild(this.canvas), this.bckStyle && (this.imgSource.style.background = this.bckStyle.background), Object.keys(this).forEach(function(a) {
        delete this[a]
    })
    ...
    this.imgDownscaled = this.customDrop || downscaleImage(this.img, 50), a.sound && playSound(a.sound);
    var d = {
        opacity: 1,
        blur: 10,
    ...
    return a.style.position = this.options.position, a.style.top = this.options.top, a.style.left = this.options.left, a.width = this.options.width, a.height = this.options.height, this.img.style.zIndex ? (a.style.zIndex = this.img.style.zIndex, this.img.style.zIndex += 1) : a.style.zIndex = 99, this.imgSource ? (this.options.parentElement.parentNode.insertBefore(a, this.imgSource), this.imgSource.style.zIndex = 100, this.imgSource.style.position = this.options.position, this.imgSource.style.top = this.options.top, this.imgSource.style.left = this.options.left, this.imgSource.style.width = this.options.width, this.imgSource.style.height = this.options.height, this.imgSource.style.background = "none", this.imgSource.style.width = this.options.width + "px") : this.options.parentElement.appendChild(a), this.options.parentElement.parentNode.style.height = this.options.height + "px", this.options.enableSizeChange && this.setResizeHandler(), a
    ...
    this.drops = b, this.requestID = window.requestAnimFrame(this.animateDrops.bind(this))
    ...
    this.reflected = document.createElement("canvas"), this.reflected.width = Math.floor(this.canvas.width / this.options.reflectionScaledownFactor), this.reflected.height = Math.floor(this.canvas.height / this.options.reflectionScaledownFactor);
    ...
    var a = this.r;
    if (this.r = Math.floor(.95 * this.r), this.r < 3) this.context.arc(this.x, this.y, this.r, 0, 2 * Math.PI, !0), this.context.closePath();
    ...
    (!a.trailY || a.y - a.trailY >= 100 * Math.random() * a.r) && (a.trailY = a.y, this.putDrop(new Drop(this, Math.floor(a.x + (2 * Math.random() - 1) * Math.random()), a.y - a.r - 5, Math.ceil(a.r / 5), 0)))
    ...
    return !!this.clearDrop(a) || (a.yspeed ? (a.yspeed += this.PRIVATE_GRAVITY_FORCE_FACTOR_Y * Math.floor(a.r), a.xspeed += Math.floor(this.PRIVATE_GRAVITY_FORCE_FACTOR_X * Math.floor(a.r))) : (a.yspeed = this.PRIVATE_GRAVITY_FORCE_FACTOR_Y, a.xspeed = Math.floor(this.PRIVATE_GRAVITY_FORCE_FACTOR_X)), a.y += Math.floor(a.yspeed), a.draw(), !1)
}, RainyDay.prototype.GRAVITY_NON_LINEAR = function(a) {
    return !!this.clearDrop(a) || (a.collided ? (a.collided = !1, a.seed = Math.floor(a.r * Math.random() * this.options.fps), a.skipping = !1, a.slowing = !1) : (!a.seed || a.seed < 0) && (a.seed = Math.floor(a.r * Math.random() * this.options.fps), a.skipping = a.skipping === !1, a.slowing = !0), a.seed--, a.yspeed ? a.slowing ? (a.yspeed /= 1.1, a.xspeed /= 1.1, a.yspeed < this.PRIVATE_GRAVITY_FORCE_FACTOR_Y && (a.slowing = !1)) : a.skipping ? (a.yspeed = this.PRIVATE_GRAVITY_FORCE_FACTOR_Y, a.xspeed = this.PRIVATE_GRAVITY_FORCE_FACTOR_X) : (a.yspeed += 1 * this.PRIVATE_GRAVITY_FORCE_FACTOR_Y * Math.floor(a.r), a.xspeed += 1 * this.PRIVATE_GRAVITY_FORCE_FACTOR_X * Math.floor(a.r)) : (a.yspeed = this.PRIVATE_GRAVITY_FORCE_FACTOR_Y, a.xspeed = this.PRIVATE_GRAVITY_FORCE_FACTOR_X), 0 !== this.options.gravityAngleVariance && (a.xspeed += (2 * Math.random() - 1) * a.yspeed * this.options.gravityAngleVariance), a.y += Math.floor(a.yspeed), a.x += Math.floor(a.xspeed), a.draw(), !1)
    ...
    a.drawImage(this.img, this.options.crop[0], this.options.crop[1], this.options.crop[2], this.options.crop[3], 0, 0, this.canvas.width, this.canvas.height), a = this.clearbackground.getContext("2d"), a.drawImage(this.img, this.options.crop[0], this.options.crop[1], this.options.crop[2], this.options.crop[3], 0, 0, this.canvas.width, this.canvas.height), !isNaN(this.options.blur) && this.options.blur >= 1 && this.stackBlurCanvasRGB(this.canvas.width, this.canvas.height, this.options.blur)
    ...
    var f, g, h, i, j, k, l, m, n, o, p, q, r, s, t, u, v, w, x, y, z = this.background.getContext("2d"),
        ...
        F = new BlurStack,
        G = E;
    for (h = 1; h < 2 * c + 1; h++) G = G.next = new BlurStack, h === C && (F = G);
    G.next = E;
    var H = null,
        I = null;
    l = k = 0;
    for (var J, K = e[c], L = 0; L < d.length; ++L)
        ...
        for (s = t = u = m = n = o = 0, p = C * (v = B[k]), q = C * (w = B[k + 1]), r = C * (x = B[k + 2]), m += D * v, n += D * w, o += D * x, G = E, h = 0; h < C; h++) G.r = v, G.g = w, G.b = x, G = G.next;
        for (h = 1; h < C; h++) i = k + ((a - 1 < h ? a - 1 : h) << 2), m += (G.r = v = B[i]) * (y = C - h), n += (G.g = w = B[i + 1]) * y, o += (G.b = x = B[i + 2]) * y, s += v, t += w, u += x, G = G.next;
        for (H = E, I = F, f = 0; f < a; f++) B[k] = m * K >> J, B[k + 1] = n * K >> J, B[k + 2] = o * K >> J, m -= p, n -= q, o -= r, p -= H.r, q -= H.g, r -= H.b, i = l + ((i = f + c + 1) < a - 1 ? i : a - 1) << 2, s += H.r = B[i], t += H.g = B[i + 1], u += H.b = B[i + 2], m += s, n += t, o += u, H = H.next, p += v = I.r, q += w = I.g, r += x = I.b, s -= v, t -= w, u -= x, I = I.next, k += 4;
        l += a
    }
    for (f = 0; f < a; f++) {
        for (t = u = s = n = o = m = 0, k = f << 2, p = C * (v = B[k]), q = C * (w = B[k + 1]), r = C * (x = B[k + 2]), m += D * v, n += D * w, o += D * x, G = E, h = 0; h < C; h++) G.r = v, G.g = w, G.b = x, G = G.next;
        for (j = a, h = 1; h < C; h++) k = j + f << 2, m += (G.r = v = B[k]) * (y = C - h), n += (G.g = w = B[k + 1]) * y, o += (G.b = x = B[k + 2]) * y, s += v, t += w, u += x, G = G.next, h < b - 1 && (j += a);
        for (k = f, H = E, I = F, g = 0; g < b; g++) i = k << 2, B[i] = m * K >> J, B[i + 1] = n * K >> J, B[i + 2] = o * K >> J, m -= p, n -= q, o -= r, p -= H.r, q -= H.g, r -= H.b, i = f + ((i = g + C) < b - 1 ? i : b - 1) * a << 2, m += s += H.r = B[i], n += t += H.g = B[i + 1], o += u += H.b = B[i + 2], H = H.next, p += v = I.r, q += w = I.g, r += x = I.b, s -= v, t -= w, u -= x, I = I.next, k += a
    }
